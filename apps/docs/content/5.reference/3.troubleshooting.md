...
- **Missing required fields** -- `version`, `ai.providers`, `ai.models.chat`, and `ai.models.browser` are all required.
...
BabyClaw uses SQLite with Prisma. The database file is created automatically at the path specified in `database.url` (default: `file:../data/babyclaw.db`).
