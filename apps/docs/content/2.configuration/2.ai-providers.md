...
BabyClaw uses up to three models for different tasks:

| Model | Required | Purpose |
|---|---|---|
| `chat` | Yes | Main conversation model -- handles messages and tool calls |
| `browser` | Yes | Used for browser automation tasks |
...
babyclaw model alias remove gpt4
```
...
```json
{
  "ai": {
    ...
    "models": {
      ...
      "browser": "openrouter:anthropic/claude-sonnet-4"
    }
  }
}
```
