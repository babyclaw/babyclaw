...
| `browser` | `string` | Yes | -- | Model for browser automation |
...
Enable browser automation tools (navigate, click, type, screenshot). Requires an MCP browser server. See [Browser Tools](/reference/browser-tools).
...
Run the browser in headless mode (no visible window). Set to `false` if you want to see what the browser is doing.
