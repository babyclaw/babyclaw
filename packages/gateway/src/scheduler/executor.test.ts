...
function createExecutor(overrides: Record<string, any> = {}) {
  return new SchedulerExecutor({
    toolDeps: {
      ...
      messageLinkRepository: overrides.messageLinkRepository ?? createMockMessageLinkRepository(),
      chatRegistry: overrides.chatRegistry ?? createMockChatRegistry(),
      deliveryService: overrides.deliveryService ?? createMockDeliveryService(),
