...
describe("SessionTitleGenerator", () => {
  ...
  it("generates a title from user message", async () => {
    ...
    const title = await generator.generate({ userMessage: "I need help debugging my postgres connection" });

    expect(title).toBe("Debug Postgres Connection");
    expect(generateText).toHaveBeenCalledOnce();
    const call = vi.mocked(generateText).mock.calls[0][0];
    expect(call.messages).toHaveLength(2);
    expect(call.messages![0]).toEqual({ role: "system", content: expect.stringContaining("4-5 word title") });
    expect(call.messages![1]).toEqual({ role: "user", content: "I need help debugging my postgres connection" });
  });
